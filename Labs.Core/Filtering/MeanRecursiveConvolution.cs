        ...
        where TPixel : struct, IColor<TPixel, TChannel>
    {
        ...
        private Accumulator SlideFrame(int pixelId, Accumulator rowSum, Frame f, ref Span<TPixel> output)
        {
            ...
            for (int y0 = y2from; y0 <= y2to; y0++)
            {
                ...
                Accumulator pixel1 = Pixels[localId1].Convert();
                Accumulator pixel2 = Pixels[localId2].Convert();
                
                oldSum = oldSum.Add(pixel1.Mul(Kernel[matrixY, matrixX1]));
                newSum = newSum.Add(pixel2.Mul(Kernel[matrixY, matrixX2]));
            }

            if (oldSum.CompareTo(newSum) == 0)
                return rowSum;

            rowSum = rowSum.Add(newSum);
            rowSum = rowSum.Subtract(oldSum);
            return rowSum;
        }
    }
}